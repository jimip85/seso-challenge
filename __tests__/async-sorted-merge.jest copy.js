...
describe("asyncSortedMerge", () => {
  ...
  test("should process multiple log sources with varying sizes", async () => {
    ...
    // Debug: Log the actual calls
    console.log("Mock Printer Calls:", mockPrinter.print.mock.calls);

    // Check that logs are printed in chronological order
    expect(mockPrinter.print.mock.calls).toEqual([
      [logEntries1[0]], // Log 1
      [logEntries2[0]], // Log 2
      [logEntries1[1]], // Log 3
    ]);
  ...
  test("should handle errors in sources", async () => {
    mockLogSources = [
      {
        popAsync: jest
          ...
          .mockRejectedValueOnce(
            new Error("Failed to fetch logs from source 0")
          ),
      },
      {
        popAsync: jest
    ...
    expect(console.error).toHaveBeenCalledWith(
      expect.stringContaining("Failed to fetch logs from source 0"),
      expect.any(Error)
    );
  });
